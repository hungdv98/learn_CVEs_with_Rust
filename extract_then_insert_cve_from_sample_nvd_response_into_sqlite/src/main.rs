...
async fn main() -> Result<()> {
    ...
    println!("Starting database upsertion (INSERT or UPDATE) ...");
    let upsert_start = std::time::Instant::now();
    upsert_cves(&pool, &extracted_list).await?;
    let upsert_duration = upsert_start.elapsed();
    println!("Upsertion time: {:.2?}", upsert_duration);

    let total_rows: i64 = sqlx::query_scalar!("SELECT COUNT(*) FROM cves")
        .fetch_one(&pool)
        .await?;
    println!("Verification: Total rows in 'cves' table: {}", total_rows);
